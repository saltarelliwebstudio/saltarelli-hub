...
import { BarChart3, Phone, Zap, ContactRound, Users, TrendingUp, Search, Calendar, Globe, Eye, Monitor, Smartphone, Tablet, ExternalLink, MousePointerClick, Clock, ArrowRight, Target } from 'lucide-react';
...
export default function AdminAnalytics() {
  ...
  const { data: eventStats, isLoading: eventsLoading } = useSiteEventStats(Number(webDays));
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Visitor → Audit → Booking</CardDescription>
          </CardHeader>
          <CardContent>
            {eventsLoading ? (
              ...
              <p className="text-center py-4 text-muted-foreground">No event data yet</p>
            ) : (
              <div className="space-y-3">
                {[
                  { label: 'Visitors', value: webStats?.uniqueVisitors || 0, color: 'bg-blue-500', pct: 100 },
                  { label: 'Audit Started', value: eventStats.auditStarts, color: 'bg-yellow-500', pct: webStats?.uniqueVisitors ? Math.round((eventStats.auditStarts / webStats.uniqueVisitors) * 100) : 0 },
                  { label: 'Audit Completed', value: eventStats.auditCompletes, color: 'bg-orange-500', pct: webStats?.uniqueVisitors ? Math.round((eventStats.auditCompletes / webStats.uniqueVisitors) * 100) : 0 },
                  { label: 'Calendly Clicked', value: eventStats.calendlyClicks, color: 'bg-green-500', pct: webStats?.uniqueVisitors ? Math.round((eventStats.calendlyClicks / webStats.uniqueVisitors) * 100) : 0 },
                ].map((step, i) => (
                  <div key={step.label}>
                    ...
                    {i < 3 && <ArrowRight className="h-3 w-3 text-muted-foreground mx-auto mt-1" />}
                  </div>
                ))}
              </div>
            )}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>How visitors interact with the site</CardDescription>
          </CardHeader>
          <CardContent>
            {eventsLoading ? (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-lg bg-blue-500/10 text-center">
                    ...
                    <p className="text-xl font-bold text-blue-500">
                      {eventStats.avgTimeOnPage >= 60
                        ? `${Math.floor(eventStats.avgTimeOnPage / 60)}m ${eventStats.avgTimeOnPage % 60}s`
                        : `${eventStats.avgTimeOnPage}s`}
                    </p>
                  ...
                  <div className="p-3 rounded-lg bg-green-500/10 text-center">
                    ...
                    <p className="text-xl font-bold text-green-500">{eventStats.conversionRate}%</p>
                    <p className="text-xs text-muted-foreground">Visitor → Audit</p>
                  </div>
                  <div className="p-3 rounded-lg bg-orange-500/10 text-center">
                    <p className="text-xl font-bold text-orange-500">{eventStats.auditToBooking}%</p>
                    <p className="text-xs text-muted-foreground">Audit → Booking</p>
                  </div>
                  <div className="p-3 rounded-lg bg-purple-500/10 text-center">
                    <p className="text-xl font-bold text-purple-500">{eventStats.ctaClicks + eventStats.calendlyClicks}</p>
                    <p className="text-xs text-muted-foreground">Total CTA Clicks</p>
                  </div>
                </div>

                {Object.keys(eventStats.sectionViews).length > 0 && (
                  <div>
                    <p className="text-sm font-medium text-muted-foreground mb-2">Sections Viewed</p>
                    <div className="space-y-1.5">
                      {Object.entries(eventStats.sectionViews)
                        .sort(([, a], [, b]) => b - a)
                        .map(([section, views]) => (
                          <div key={section} className="flex items-center justify-between text-sm">
                            <span className="capitalize text-foreground">{section.replace(/-/g, ' ')}</span>
